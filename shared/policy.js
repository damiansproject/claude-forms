...
function policySummary() {
  return [
    ...
    'Readable code over fortress code; robustness/security only when requested. Prefer rewrite over patch stacks. Minimal files ≠ done: honest types, owned edges, tests for non-trivial branching, architecture obvious (file-top note, marked entrypoint, fewer helpers, comment regex/magic), always format touched files (add minimal prettier on new JS/TS); delete one-offs; keep docs true.',
...
function userPromptReminder() {
  return (
    ...
    'make architecture obvious and always format touched files before finishing.'
  );
}
...
function stopGroundingReminder() {
  return (
    ...
    'Before finishing: exercise edges you own, avoid type cast stacks, run or add focused tests for non-trivial branching, ' +
    'make architecture obvious (file-top orientation, marked entrypoint, comment non-obvious regex/magic, avoid helper spam), ' +
    'delete one-off scripts you created, update docs your change made wrong, and always format touched files ' +
    '(language-standard formatter; add minimal prettier/`format` on new JS/TS if missing). ' +
